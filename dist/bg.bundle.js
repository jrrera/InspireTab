...
/***/ function(module, exports) {
	...
	function getProductivityScore(counts) {
	  var score = 100,
	      ...
	      penaltyMultiplier = 3,
	      // Each subsequent visit over the threshold has a penalty multiplier
	    ...
	    if (urlPatterns.hasOwnProperty(site)) {
	      if (changeInfo.url && changeInfo.url.search(urlPatterns[site]) > -1) {
	        ...
	        chrome.tabs.update(tabId, {
	          url: 'ext/components/interrupt/interrupt.html?redirect=' + encodeURIComponent(changeInfo.url) + '&site=' + site + '&count=' + (counts[site] || 0) + '&minutes=' + disableDurationInMinutes + '&score=' + getProductivityScore(counts)
	        });
	      }
	    }
	  }
